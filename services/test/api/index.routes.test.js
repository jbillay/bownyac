...
describe("routes: not Found", () => {
  test("should respond as expected", async () => {
    ...
    expect(response.type).toEqual("application/json");
    expect(response.body.message).toEqual('No endpoint matched your request: GET /notFound');
  });
});
...
describe("routes: Error with Database", () => {
  test("should respond as expected", async () => {
    await expect(server.create(configWrong))
      .rejects
      .toThrow('Unable to connect to the database');;
  });
});
